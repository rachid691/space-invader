...
class Aliens:
    """
    Classe qui gère le type des aliens, leurs déplacements
    """
    ...
    def __init__(self, canvas, game, Horde, x_pos=6, y_pos=1, direction = 1, speed = 2, frequence = 16, kind=2):
        ...
        self.canvas = canvas
        self.x_pos = x_pos
        self.set_image = [tk.PhotoImage(file='mechant1.png'),
                          tk.PhotoImage(file='mechant2.png'),
                          tk.PhotoImage(file='mechant22.png')] #50px*50px
        ...
        if self.pv == 3:
            self.pv = 1
            # C'est l'ennemi bonus
 
    def mouvement_ennemi_bonus(self):
    # Si on ajoute un ennemi bonus dans le jeu
        if self.game.game_over:
            return
        if self.canvas.coords(self.sprite)[0] + self.direction*self.speed > 0 and self.canvas.coords(self.sprite)[0] + self.direction*self.speed + 50 < 600:
            self.canvas.move(self.sprite, self.direction*self.speed, 0)
        else:
            self.direction = (self.direction == -1) - (self.direction == 1)
            self.canvas.move(self.sprite, self.direction*self.speed, 0)
        self.canvas.after(self.frequence,self.mouvement_ennemi_bonus)
 
    def mouvement_surprise(self):
        if self.game.game_over:
            return
        if self.canvas.coords(self.sprite)[0] + self.direction*self.speed > 0 and self.canvas.coords(self.sprite)[0] + self.direction*self.speed + 50 < 650:
            self.canvas.move(self.sprite, self.direction*self.speed, 0)
        else:
            self.canvas.destroy(self.sprite)
        self.canvas.after(self.frequence,self.mouvement_surprise)
 
class Horde:
    """
    Classe qui gère les aliens en tant que groupe
    """
    ...
    def mouvements(self):
        ...
        if mouvement_ok:
            ...
            for Aliens in self.aliens_list:
                ...
                Aliens.direction = (Aliens.direction == -1) - (Aliens.direction == 1) # Pas utilisé dans horde mais fait par rigueur
            self.direction = (self.direction == -1) - (self.direction == 1)
...
class Shot:
    """
    Classe qui gère les tirs du joueur et des aliens
    """
...
class Vaisseau:
    """
    Classe qui gère les déplacements du joueur
    """
...
class Menu:
    ...
    def __init__(self):        
        ...
        self.replay_button = tk.Button( text = 'REPLAY !',height = 4, width = 20,activebackground='#ECECEC',background='#FFFFFF')
 
 
 
class Jeu:
    """
    Classe principale : objet gérant la fenêtre de jeu, le canvas, et a pour attributs les autres objets du programme.
    """
    ...
    def record(self): # Gère la modification en direct de la valeur du highscore après une partie
    ...
    def win(self): # Ecran de victoire
        self.round_SV.set('VICTOIRE')
    ...
    def shots_management(self): # Gestion du mouvement des lasers et de la collision, ainsi que de l'augmentation de la variable score
        ...
        for shot in self.current_shots:
            ...
            if len(self.horde.aliens_list) == 0 and len(self.current_shots) == 0:
                self.canvas.after(1000, self.win) # on lance la prochaine manche
                return
...
class Brique:
    """
    Objet qui compose les murs : carré blanc
    """
...
if __name__ == '__main__':
    window = tk.Tk() # Initialisation de la variable qui gère la fenêtre de jeu
